...
const renderMap = async (req, res) => {
    try {
        if(req.params.sc == coffee){
            ...
            res.status(404).render('404notFound', { message: req.originalUrl });
        }
    } catch (err) {
...
const renderAnlysis = async (req, res) => {
    try {
        if(req.params.sc == coffee){
            ...
            res.render('graph.hbs',{id:climate_id})
        }
        else if (req.params.sc == health){
            res.render('graph.hbs',{id:health_id})
        }
        else if (req.params.sc == unemployment){
            res.render('graph.hbs',{id:unemployment_id})
        }
        else {
            res.status(404).render('404notFound', { message: req.originalUrl });
        }
